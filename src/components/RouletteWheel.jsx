...
const RouletteWheel = ({ participants, onWinnerSelected }) => {
  ...
  // Al montar el componente, establecer segmentos de ruleta
  useEffect(() => {
    // Usar 12 segmentos para una ruleta más simple y clásica
    const displayCount = 12;
    setDisplayParticipants(Array(displayCount).fill(null));
  }, [participants]);
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      ...
      {/* Ruleta - diseño clásico y más simple */}
      <div className="relative w-[320px] h-[320px] sm:w-[420px] sm:h-[420px] mb-12">
        {/* Borde exterior */}
        <div className="absolute inset-0 rounded-full overflow-hidden border-4 border-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.4)]"></div>
        
        {/* Marcador en la parte superior */}
        <div className="absolute -top-0.5 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-none">
          <div className="w-0 h-0 border-l-[12px] border-r-[12px] border-b-[20px] border-l-transparent border-r-transparent border-b-pink-500 filter drop-shadow-[0_0_5px_rgba(236,72,153,0.8)]"></div>
        </div>
        ...
        >
          {/* Segmentos simples alternando colores */}
          {displayParticipants.map((_, index) => {
            // Colores alternados clásicos de ruleta
            const isEven = index % 2 === 0;
            const bgColor = isEven ? 'bg-purple-800' : 'bg-purple-700';
            ...
            return (
              <div 
                key={`segment-${index}`}
                className={`absolute top-0 left-0 w-full h-full ${bgColor} origin-center`}
                style={{ 
                  transform: `rotate(${rotation}deg)`,
                  clipPath: `polygon(50% 50%, 50% 0, ${50 + 50 * Math.cos(Math.PI * (index + 1) * 2 / displayParticipants.length)}% ${50 + 50 * Math.sin(Math.PI * (index + 1) * 2 / displayParticipants.length)}%)`,
                  borderRight: '1px solid rgba(255, 255, 255, 0.1)'
                }}
              >
                {/* Línea divisoria simple */}
                <div className="absolute top-0 left-1/2 w-[1px] h-[100%] bg-white/20"></div>
              </div>
            );
          })}
          
          {/* Centro con logo Prodispro */}
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90px] h-[90px] sm:w-[120px] sm:h-[120px] rounded-full bg-purple-900 z-10 flex items-center justify-center border-2 border-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.3)]">
      ...
      <button
        ...
        className={`
          ...
          ${!isSpinning ? 
            'bg-gradient-to-r from-yellow-400 to-pink-500 text-white shadow-lg hover:-translate-y-1 transition-all duration-300' : 
