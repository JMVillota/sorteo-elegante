...
const WinnersScreen = ({ winners, onReset }) => {
  ...
  useEffect(() => {
    console.log('🏆 WinnersScreen - Ganadores recibidos:', winners);
    console.log('📊 Total ganadores:', winners.length);
    console.log('🎁 Ganadores por premio:', winners.reduce((acc, winner) => {
  ...
  useEffect(() => {
    ...
    // Opcional: detener confeti después de 10 segundos
  ...
  // Función para generar PDF elegante CORREGIDA
  const generatePDF = async () => {
    setIsGeneratingPDF(true);
    
    try {
      console.log('📄 Generando PDF con ganadores:', winners.length);
      
      // VERIFICAR que solo procesamos ganadores reales
      if (winners.length === 0) {
  ...
  // Generar contenido HTML para el PDF CORREGIDO
  const generatePDFContent = () => {
    ...
    // CORREGIDO: Agrupar SOLO los ganadores reales por premio
    const groupedWinners = winners.reduce((acc, winner) => {
      // Verificar que es un ganador válido
      if (!winner || !winner.prize || !winner.participant) {
        console.warn('⚠️ Ganador inválido encontrado:', winner);
        return acc;
      }
    ...
    console.log('📊 PDF - Ganadores agrupados por premio:', groupedWinners);
      ...
      <div class="container">
        ...
        <div class="footer">
          ...
          <p class="footer-text">
            ...
            <strong>Total de ganadores: ${winners.length}</strong>
          </p>
        </div>
      </div>
    </body>
  ...
  // CORREGIDO: Agrupar ganadores por premio para mostrar
  const groupedWinners = winners.reduce((acc, winner) => {
    // Verificar que es un ganador válido
    if (!winner || !winner.prize || !winner.participant) {
  ...
  return (
    <div className="min-h-screen bg-prodispro-black relative overflow-hidden">
      {/* Confeti de celebración - FORZADO A MOSTRAR */}
      {showConfetti && (
        <Confetti
          width={width}
          height={height}
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        ...
        <div className="text-center mb-12 relative">
          ...
          <div className="relative bg-prodispro-gray/90 backdrop-blur-sm rounded-3xl p-8 border border-prodispro-blue/30">
            ...
            {/* DEBUG INFO */}
            <div className="mb-6 p-4 bg-prodispro-blue/10 rounded-xl border border-prodispro-blue/30">
              <p className="text-sm text-gray-400">
                Total de ganadores REALES: {winners.length} • 
                Premios con ganadores: {Object.keys(groupedWinners).length}
              </p>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <button
                onClick={generatePDF}
                disabled={isGeneratingPDF || winners.length === 0}
              ...
              >
                <span>📄</span>
                <span>{isGeneratingPDF ? 'Generando...' : `Certificado PDF (${winners.length})`}</span>
                {isGeneratingPDF && (
        ...
        {winners.length === 0 ? (
          ...
          <>
            ...
            {/* Lista de ganadores por premio - Solo ganadores REALES */}
           ...
           {/* Tabla completa de ganadores - SOLO GANADORES REALES */}
           <div className="bg-gradient-to-br from-prodispro-gray/90 to-prodispro-light-gray/90 backdrop-blur-sm rounded-3xl p-8 border border-prodispro-blue/30 mb-12">
           ...
           <div className="text-center">
             <div className="inline-block p-8 bg-gradient-to-br from-prodispro-gray/90 to-prodispro-light-gray/90 backdrop-blur-sm rounded-3xl border border-prodispro-blue/30 relative overflow-hidden">
               ...
               <div className="relative z-10">
                 ...
                 <h3 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-prodispro-blue to-green-400 mb-4">
                   ¡Gracias por participar!
                 </h3>
                 <p className="text-gray-300 mb-8 text-lg">
                   El sorteo PRODISPRO 2025 ha concluido exitosamente<br/>
                   {winners.length} ganador{winners.length > 1 ? 'es han' : ' ha'} sido seleccionado{winners.length > 1 ? 's' : ''} de manera transparente y aleatoria
                 </p>
                 ...
                 <div className="flex flex-wrap justify-center gap-4">
                   <button
                     ...
                     className="px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold rounded-xl transition-all duration-300 hover:scale-105 transform flex items-center space-x-2 disabled:opacity-50"
                   >
                     <span>📋</span>
                     <span>{isGeneratingPDF ? 'Generando Certificado...' : `Certificado PDF (${winners.length} ganadores)`}</span>
                     {isGeneratingPDF && (
                   ...
                   <button
                     onClick={onReset}
                     className="px-8 py-4 bg-gradient-to-r from-prodispro-blue to-cyan-500 hover:from-prodispro-blue/80 hover:to-cyan-500/80 text-white font-bold rounded-xl transition-all duration-300 hover:scale-105 transform flex items-center space-x-2"
                   >
                     <span>🚀</span>
                     <span>Iniciar Nuevo Sorteo</span>
                   </button>
                 </div>
                 ...
                 <div className="mt-8 p-4 bg-prodispro-blue/10 rounded-xl border border-prodispro-blue/30">
                   <p className="text-sm text-gray-400">
                     Sorteo realizado el {new Date().toLocaleDateString('es-ES', {
                       ...
                       hour: '2-digit',
                       minute: '2-digit'
                     })} • {winners.length} ganadores reales verificados
                   </p>
                 </div>
               </div>
             </div>
     ...
     {/* Efectos de partículas flotantes mejorados */}
